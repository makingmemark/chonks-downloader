...
const svgOutputPath = path.join('./output-trait-svg', `${tokenId}.svg`);
const pngOutputPath = path.join('./output-trait-png', `${tokenId}.png`);
const ABI = [
...
if (!fs.existsSync('./output-trait-svg')) {
    fs.mkdirSync('./output-trait-svg');
}
if (!fs.existsSync('./output-trait-png')) {
    fs.mkdirSync('./output-trait-png');
}
